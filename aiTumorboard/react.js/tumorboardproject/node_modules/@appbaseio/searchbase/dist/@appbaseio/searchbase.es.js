...
var getSuggestions = function getSuggestions(fields, suggestions, value, showDistinctSuggestions, enablePredictiveSuggestions) {
  ...
  var getPredictiveSuggestions = function getPredictiveSuggestions(_ref) {
    var predictiveSuggestions = _ref.predictiveSuggestions,
        currentValuePredictive = _ref.currentValuePredictive,
        wordsToShowAfterHighlight = _ref.wordsToShowAfterHighlight;
    var suggestionMap = {};
    var currentValuePredictiveTrimmed = currentValuePredictive.trim();

    if (currentValuePredictiveTrimmed) {
      var parsedSuggestion = predictiveSuggestions.reduce(function (agg, _ref2) {
        ...
        var regexString = "^(" + currentValuePredictiveTrimmed + ")\\w+";
        ...
        if (regexExecution) {
          ...
          var suggestionPhrase = currentValuePredictiveTrimmed + "<mark class=\"highlight-class\">" + matchedString.slice(currentValuePredictiveTrimmed.length).split(' ').slice(0, wordsToShowAfterHighlight + 1).join(' ') + "</mark>"; // to show unique results only

          if (!suggestionMap[suggestionPhrase]) {
            suggestionMap[suggestionPhrase] = 1;
            return [].concat(agg, [_extends({
  ...
  function SearchComponent(_ref) {
    ...
    _defineProperty(_assertThisInitialized(_this), "getRecentSearches", function (queryOptions, options) {
      ...
      return new Promise(function (resolve, reject) {
        fetch(_this.url + "/_analytics/" + _this.index + "/recent-searches?" + queryString, requestOptions).then(function (res) {
          if (res.status >= 500) {
            return reject(res);
          }
          ...
          return res.json().then(function (recentSearches) {
            ...
            _this._applyOptions({
              stateChanges: options.stateChanges
            }, 'recentSearches', prev, _this.recentSearches); // Populate the recent searches
  ...
  _proto._fetchRequest = function _fetchRequest(requestBody, isPopularSuggestionsAPI) {
    ...
    return new Promise(function (resolve, reject) {
      _this2._handleTransformRequest(requestOptions).then(function (finalRequestOptions) {
        ...
        var index = isPopularSuggestionsAPI ? '.suggestions' : _this2.index;
